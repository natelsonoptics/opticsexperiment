...
class HeatingPolarization(PolarizationMeasurement):
    ...
    def do_measurement(self, polarization):
        ...
        self._writer.writerow([time_now, polarization, raw[0], raw[1], iphoto[0], iphoto[1]])
        self._ax1.plot(conversions.degrees_to_radians(polarization), abs(iphoto[0]) * 1000, linestyle='',
                       color='blue', marker='o', markersize=2)
        self._ax1.set_rmax(self._vmax_x * 1.1 * 1000)
        self._ax2.plot(conversions.degrees_to_radians(polarization), abs(iphoto[1]) * 1000, linestyle='',
                       color='blue', marker='o', markersize=2)
