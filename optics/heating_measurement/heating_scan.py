...
class HeatingScan:
    def __init__(self, filepath, notes, device, scan, gain, bias, osc, xd, yd, xr, yr, xc, yc, polarization,
        ...
        self.im1 = self.ax1.imshow(self.z1.T, cmap=plt.cm.coolwarm, interpolation='nearest', origin='lower')
        self.im2 = self.ax2.imshow(self.z2.T, cmap=plt.cm.coolwarm, interpolation='nearest', origin='lower')
    ...
    def write_header(self):
        ...
        self.writer.writerow(['x_raw', 'y_raw', 'x_iphoto', 'y_iphoto', 'x_pixel', 'y_pixel'])

    def setup_plots(self):
        self.clb1.set_label('current (mA)', rotation=270, labelpad=20)
        self.clb2.set_label('current (mA)', rotation=270, labelpad=20)
        self.ax1.title.set_text('iphoto X')
        self.ax2.title.set_text('iphoto Y')
    ...
    def main(self):
        ...
        with open(self.file, 'w', newline='') as inputfile:
            try:
                ...
                self.setup_plots()
                self.fig.show()
                self.sr7270_top.change_applied_voltage(self.bias)
                time.sleep(0.3)
                self.sr7270_top.change_oscillator_amplitude(self.osc)
                self.write_header()
                self.run_scan()
                heating_plot.plot(self.ax1, self.im1, self.z1, np.amax(np.abs(self.z1)), -np.amax(np.abs(self.z1)))
                heating_plot.plot(self.ax2, self.im2, self.z2, np.amax(np.abs(self.z2)), -np.amax(np.abs(self.z2)))
                ...
                self.sr7270_top.change_applied_voltage(0)
