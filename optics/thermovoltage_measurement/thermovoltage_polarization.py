...
class ThermovoltagePolarization(PolarizationMeasurement):
    def __init__(self, master, filepath, notes, device, scan, gain, npc3sg_input, sr7270_single_reference, powermeter,
                 waveplate, steps):
        super().__init__(master, filepath, notes, device, scan, gain, npc3sg_input, sr7270_single_reference, powermeter,
                         waveplate, steps)
    ...
    def do_measurement(self, polarization):
        ...
        self._writer.writerow([time_now, polarization, raw[0], raw[1], voltages[0], voltages[1]])
        self._ax1.plot(conversions.degrees_to_radians(polarization), abs(voltages[0]) * 1000000,
                       linestyle='', color='blue', marker='o', markersize=2)
        self._ax1.set_rmax(self._vmax_x * 1000000 * 1.1)
        self._ax2.plot(conversions.degrees_to_radians(polarization), abs(voltages[1]) * 1000000,
                       linestyle='', color='blue', marker='o', markersize=2)
